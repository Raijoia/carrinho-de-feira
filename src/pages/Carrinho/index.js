...
function Carrinho() {
  ...
  // se não existir saldo, vai como padrão para 0
  const { saldo = 0 } = useContext(UsuarioContext)
  const { formaPagamento, mudarFormaPagamento, tiposPagamento } = usePagamentoContext()

  const history = useHistory()

  // useMemo usado para carregar o total apenas se o saldo ou o valor total mudar 
  const total = useMemo(() => saldo - valorTotalCarrinho, [saldo, valorTotalCarrinho])
  
  return (
    <Container>
      <Voltar
        // goBack vai para a tela anterior que estava navegando
